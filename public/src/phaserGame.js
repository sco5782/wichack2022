...
function create ()
{
    bg_tiles = this.add.group({key: 'bgtile', repeat: 48, setScale: {x: 0.15, y: 0.15 }});
    bad_tiles = this.add.group({key: 'tile', repeat: 2, setScale: {x:0.15, y:0.15 }});
    good_tile = this.add.image(0, 0, 'tile').setScale(0.15);
    bad_tiles.add(good_tile);

    var grid_x = (this.game.config.width / 2) - 460;
    var tile_size = 75;

    bad_tiles.getChildren().forEach(function(tile){
        tile.x = ((Math.round(Math.random() * 6) + 1) * tile_size + 3);
        tile.y = ((Math.round(Math.random() * 6) + 1) * tile_size - 12);
        tile.setInteractive();
    }, this);

    Phaser.Actions.GridAlign(bg_tiles.getChildren(), { width: 7, cellWidth: tile_size, cellHeight: tile_size, x: grid_x, y: -150 });
    ...
    bad_tiles.getChildren().forEach(function(tile){
        tile.on("pointerover", function(){
            ...
            d_text.alpha = 0;
            d_text.setText(discouraging[Math.round(Math.random() * 3)]);
            this.tweens.add({
                targets: d_text,
                alpha: 1,  
                duration: 500,
